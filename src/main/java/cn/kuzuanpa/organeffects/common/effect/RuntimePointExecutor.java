...
public final class RuntimePointExecutor {
    ...
    private static void applyExecution(Player player, IEffectHolder holder, EffectInstance instance, EffectDefinition.BonusAction execution) {
        PointExecutor executor = OrganEffectsExtensionApi.getPointExecutor(execution.type());
        if (executor == null) {
            OrganEffectsDebug.trace(player, "execution missing type=%s", execution.type());
            return;
        }
    ...
    public static PointExecutor.PointUsage consumePointUsage(Player player, IEffectHolder holder, EffectDefinition.BonusAction execution) {
        if (execution.pointType() == null || execution.pointId() == null) {
            OrganEffectsDebug.trace(player, "resolve usage skipped missing point binding");
            return new PointExecutor.PointUsage(0L);
        }
        long maxConsume = Math.max(0L, execution.maxConsume());
        if (maxConsume <= 0L) {
            OrganEffectsDebug.trace(player, "resolve usage skipped maxConsume=%d", execution.maxConsume());
            return new PointExecutor.PointUsage(0L);
        }
        String pointKey = execution.pointType() + ":" + execution.pointId();
        long runtimeAvailable = holder.getRuntimePoints().getOrDefault(pointKey, 0L);
        if (runtimeAvailable > 0L) {
            ...
            if (execution.isPointsConsume()) {
                OrganEffectsDebug.trace(player, "resolve runtime %s available=%d used=%d remaining=%d consume=%s", pointKey, runtimeAvailable, used, remaining, execution.isPointsConsume());
            }
        ...
        if (execution.isPointsConsume()) {
            OrganEffectsDebug.trace(player, "resolve pooled %s source=%s available=%d used=%d remaining=%d consume=%s",
