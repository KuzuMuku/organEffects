package cn.kuzuanpa.organeffects.client.screen;

import cn.kuzuanpa.organeffects.common.network.OrganEffectsNetwork;
import cn.kuzuanpa.organeffects.common.network.SelectSkillC2SPacket;
...
public final class SkillWheelScreen {
    ...
    private final Component title = Component.translatable("screen.organeffects.skill_wheel");
    ...
    public void render(GuiGraphics guiGraphics, int width, int height, int mouseX, int mouseY) {
        ...
        guiGraphics.flush();
        if (skills.isEmpty()) {
            drawCenteredLabel(guiGraphics, font, Component.translatable("message.organeffects.skill.none"), centerX, centerY + 10, 0xFF8080);
            return;
        }
    ...
    public void applySelection() {
        if (!skills.isEmpty()) {
            ...
            OrganEffectsNetwork.sendToServer(new SelectSkillC2SPacket(skillId));
        }
    }
    ...
    private Component getCooldownStatus(long cooldownTicks) {
        if (cooldownTicks <= 0L) {
            return Component.translatable("screen.organeffects.skill_wheel.ready");
        }
        return Component.translatable("message.organeffects.skill.cooldown", formatSeconds(cooldownTicks));
    }
