...
public final class BeamEffectRenderer {
    ...
    public static void onRenderLevelStage(RenderLevelStageEvent event) {
        ...
        for (BeamState state : ACTIVE_BEAMS.values()) {
            ...
            if (state.kind() == OrganEffectsNetwork.BeamEffectKind.END_CRYSTAL && source instanceof EndCrystal crystal) {
                renderEndCrystalBeam(crystal, target, partialTick, poseStack, buffer);
            } else if (state.kind() == OrganEffectsNetwork.BeamEffectKind.GUARDIAN) {
    ...
    private record BeamKey(OrganEffectsNetwork.BeamEffectKind kind, int sourceEntityId) {
    }

    private record BeamState(OrganEffectsNetwork.BeamEffectKind kind, int sourceEntityId, int targetEntityId, long startedAtTick, long expiresAtTick) {
    }
}
