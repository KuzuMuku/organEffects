...
description: Guide KubeJS developers to add OrganAPI/OrganEffects-compatible organs using plain items plus organ JSON/effects.
---

# KubeJS organ development

Use this for adding OrganEffects-compatible organs from KubeJS without writing a custom Java `OrganItem`.

## What this flow means

- KubeJS registers a plain item.
- OrganAPI resolves the organ from `data/<namespace>/organapi/organs/<id>.json`.
- OrganEffects reads that organ's `effects[]`.
...
2. If OrganEffects or OrganAPI public signatures changed, run `cd ../organEffects && ./gradlew devJar` and/or `cd ../organAPI && ./gradlew devJar` before compiling this addon so the flat-dir dependency jars in `build/libs` are refreshed.
...
- Tooltip is missing OrganEffects info: `effects[]` did not parse, or the point/skill localization keys are missing.
...
- `src/main/resources/data/organeffects/organapi/organs/wonder_taunt_core.json`
